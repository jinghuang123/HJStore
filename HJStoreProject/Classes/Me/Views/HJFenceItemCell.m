...
- (void)updateContentWithFenceModel:(HJFenceModel *)fence {
    [_userIcon sd_setImageWithURLString:fence.avatar placeholderImage:PLACEHOLDER_HEAD];
    _userName.text = fence.nickname;
    NSString *preMobile = [fence.mobile substringToIndex:3];
    NSString *tailMobile = fence.mobile.length > 8 ? [fence.mobile substringFromIndex:8] : @"";
    _phoneNumLabel.text = [NSString stringWithFormat:@"%@****%@",preMobile,tailMobile];
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:fence.createtime];
    _dateLabel.text = [date jk_formatWithTimeZoneWithoutTime:[NSTimeZone localTimeZone]];
}
